-- Adds a separately managed issue date while preserving created_at fallback for old records.
-- Review and run only in the dedicated website-admin Supabase project.

alter table public.warranties
  add column if not exists issued_date date;

drop function if exists public.create_warranty_record(
...
begin
  ...
  if input_issued_date is null then
  ...
  insert into public.warranty_daily_sequences (registration_date, last_sequence)
  values (v_registration_date, 1)
  ...
  insert into public.warranties (
    id, warranty_number, customer_name, phone, region, place,
    installation_date, issued_date, product_name, installation_area,
    warranty_period, installer, notes, created_by, updated_by
  ) values (
    v_generated_id, v_generated_number, input_customer_name, input_phone,
    input_region, input_place, input_installation_date, input_issued_date,
    input_product_name, input_installation_area, input_warranty_period,
    input_installer, nullif(input_notes, ''), auth.uid(), auth.uid()
  );
...
revoke all on function public.create_warranty_record(
  text, text, text, text, date, date, text, text, text, text, numeric
) from public;
